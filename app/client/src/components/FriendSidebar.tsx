...
export function FriendSidebar() {
    ...
    const [openDmPeers, setOpenDmPeers] = useState<number[]>([]);

    // DM 창 열기 함수
    const openDm = (peerId: number) => {
        if (!openDmPeers.includes(peerId)) {
            setOpenDmPeers((prev) => [...prev, peerId]);
        }
    };

    // DM 창 닫기 함수
    const closeDm = (peerId: number) => {
        setOpenDmPeers((prev) => prev.filter((id) => id !== peerId));
    };
    ...
    return (
        <>
                    ...
                    <div className="flex-1 overflow-y-auto p-2">
                        ...
                        {friends.map((f) => (
                            ...
                            >
                                ...
                                {/* 이름 + 상태 (여기가 DM 열기 영역) */}
                                <button
                                    // ✅ 기존 navigate 대신 팝업 상태를 변경하는 함수 호출
                                    onClick={() => {
                                        openDm(Number(f.uid));
                                        // closeRightPanel(); // 팝업창을 여러 개 띄우기 위해 사이드바를 닫지 않도록 주석 처리
                                    }}
            ...
            {/* ✅ 페이스북 스타일: 다중 DM 팝업 렌더링 영역 */}
            {/* fixed 속성 덕분에 사이드바 영역 밖으로 빠져나와 화면 우측 하단에 고정됩니다. */}
            <div className="fixed bottom-16 right-6 flex items-end gap-4 z-50 pointer-events-none">
                {openDmPeers.map(peerId => {
                    const friendData = friends.find(f => Number(f.uid) === peerId);

                    return (
                        <div key={peerId} className="w-[320px] h-[450px] bg-white rounded-t-xl shadow-[0_0_20px_rgba(0,0,0,0.15)] flex flex-col overflow-hidden pointer-events-auto border border-gray-200 animate-fade-in">
                            {/* 팝업 헤더 (창 틀) */}
                            <div className="bg-[#5CC87A] px-3 py-2.5 flex justify-between items-center text-white shrink-0">
                                <div className="flex items-center gap-2">
                                    <div className="w-6 h-6 rounded-full bg-white/20 flex items-center justify-center text-xs font-bold">
                                        {friendData?.avatar ?? friendData?.name?.charAt(0) ?? '?'}
                                    </div>
                                    <span className="font-bold text-sm">{friendData?.name || `유저 ${peerId}`}</span>
                                </div>
                                {/* 닫기 버튼 */}
                                <button
                                    onClick={() => closeDm(peerId)}
