...
export function DirectMessagesPage() {
  ...
  // 1. 내 정보 가져오기 (메시지 좌우 정렬 기준)
  // user 객체 안에 id 필드가 있다고 가정합니다.
  const { user: myInfo } = useAuthStore();

  // 2. DM 커스텀 훅 연결
  // 화면 진입 시 자동으로 히스토리를 불러오고 STOMP 구독이 시작됩니다.
  const { dmMessages, sendMessage } = useDmMessage(peerId);

  // 상단 헤더용 더미 유저 정보 (추후 사용자 조회 API로 교체 필요)
  ...
  const handleSend = () => {
    if (message.trim()) {
      sendMessage(message); // STOMP로 메시지 전송
      setMessage("");
    }
  };
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="h-14 border-b border-gray-100 px-6 flex items-center justify-between bg-white">
          <div className="flex items-center gap-3">
            <div className={`w-10 h-10 rounded-full ${currentUser?.color || 'bg-gray-400'} flex items-center justify-center text-white font-bold`}>
              {currentUser?.name.charAt(0) || '?'}
            </div>
            <div>
              <h2 className="font-bold text-[#2C3E50]">{currentUser?.name || '알 수 없는 사용자'}</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          {/* 4. 더미 데이터 대신 dmMessages를 순회하여 렌더링 */}
          {dmMessages.map((msg, index) => {
            // STOMP DTO 스펙에 맞춰 필드명(senderId, content 등)을 변경해야 할 수 있습니다.
            ...
            return (
                ...
                >
                  {!isMe && (
                      <div className={`w-10 h-10 rounded-full ${currentUser?.color || 'bg-gray-400'} flex items-center justify-center text-white font-bold flex-shrink-0`}>
                        {currentUser?.name.charAt(0) || '?'}
                      </div>
                  )}
                  <div className={`max-w-md ${isMe ? "text-right" : ""}`}>
                    <div className="flex items-baseline gap-2 mb-1">
                      ...
                      {/* 시간 포맷팅은 msg.createdAt 등을 사용해 별도로 처리 필요 */}
                      <span className="text-xs text-gray-400">{msg.createdAt || '방금 전'}</span>
                    </div>
                    <div
        ...
        <div className="p-4 border-t border-gray-100">
          <div className="flex gap-2">
            <input
                ...
                placeholder={`${currentUser?.name || '상대방'}님에게 메시지 보내기...`}
